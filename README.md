...
Скиллы лежат в [.agents/skills/](.agents/skills/) — Codex подхватывает их автоматически, когда открыта папка репозитория:

- **по одному на каждую программу** — рабочие процессы и типовые грабли;
- **self-learning + learned-rules** — агент запоминает твои правки и найденные грабли, со временем работает точнее;
- **brand-kit** — заполни [brand/BRAND.md](brand/BRAND.md) (цвета, шрифты, тон) один раз — и все работы будут в фирменном стиле;
- **cross-app** — сквозные задачи через несколько программ (лого в Illustrator → анимация в AE → монтаж в Premiere) без потери качества на стыках;
- **design-review** — структурное ревью макета с конкретными правками перед финальным экспортом.
